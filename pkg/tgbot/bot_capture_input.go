...
func (b *bot) captureNameResult(value, noun string, err error, retry func(string) stateFn, name func() string) stateFn {
	if err != nil {
		return b.captureRetry(fmt.Sprintf("coulding find %s matching `%s`", noun, value), retry)
	}
	b.queryState.selectionAttempts = 0
