...
func withPaddingToFive(in string) (string, string) {
	if len(in) >= 5 {
		return in, ""
	}
	return in, strings.Repeat(" ", max(0, 5-len(in)))
}

func (lsr LeaderSongRank) LineItem() *mdwn.Builder {
	mb := mdwn.MakeBuilder(1024).PushString("📃 ")
	mb.Preformatted(withPaddingToFive(lsr.PageNum)).Italic(lsr.SongTitle).Text(" (", lsr.Key, "): ")
	mb.Text("**", lsr.NumLeads, "** times")
	return mb
}
...
func (r LeaderOfSongInfo) LineItem() *mdwn.Builder {
	...
	if r.NumYears == 1 {
		mb.Text(" leads, in ").PushInt(r.NumYears)
		mb.PushString(" year")
	} else {
	...
	if r.LedInLastYear {
		mb.Text(", including in the last year.")
	} else {
		mb.PushString(".")
	}
	return mb
...
func (r SingingLessionInfo) LineItem() *mdwn.Builder {
	...
	mb.TextWords(".", r.SingerName, "led", r.SongPageNumber, "(").Italic(r.SongName).Text("; ", r.SongKey, ")")
	return mb
}
...
func (r LeaderLeadCount) LineItem() *mdwn.Builder {
	mb := mdwn.MakeBuilder(1024).PushString("✋ ").Bold(r.Name, ": ")
	mb.PushInt(r.Count)
...
func (lf LeaderFootstep) LineItem() *mdwn.Builder {
	mb := mdwn.MakeBuilder(1024).PushString("👣 ").Bold(withPaddingToFive(lf.SongPage)).Italic(lf.SongTitle).Text(" (", lf.SongKeys, ") led ")
...
func (r SingingBuddy) LineItem() *mdwn.Builder {
	...
	return mb.Text(" shared singings")
}
...
func (r LeaderSingingsInYear) LineItem() *mdwn.Builder {
	mb := mdwn.MakeBuilder(1024).PushString("📅 ").Bold(r.Year, ": ")
