...
func TestActiveSingingStrangers(t *testing.T) {
	t.Parallel()
	conn, ctx := testConn(t)
	count := 0
	for _, err := range conn.ActiveSingingStrangers(ctx, testSocialLeader, 5) {
		if err != nil {
			t.Fatal(err)
		}
		count++
	...
	if count == 0 {
		t.Errorf("ActiveSingingStrangers(%q): expected at least one result", testSocialLeader)
	}
}
